...
def hrf(
    t,
    a1=6.0,
    b1=1.0,  # main positive peak: shape & scale
    a2=16.0,
    b2=1.0,  # post-stimulus undershoot: shape & scale
    c=0.1,  # undershoot ratio
    a0=1.5,
    b0=0.5,
    d=0.08,  # initial-dip term: shape, scale, amplitude
):
    """
...
def plot_hrf_time():
    ...
    ax.fill_between(t, h, 0, where=(h > 0), alpha=0.15, color=plot_config.CONCEPT_PALETTE["accent_a"])
    ax.fill_between(t, h, 0, where=(h < 0), alpha=0.15, color=plot_config.CONCEPT_PALETTE["accent_b"])
...
def plot_hrf_spectrum():
    ...
    ax.axhline(
        -3, color=plot_config.CONCEPT_PALETTE["accent_b"], linewidth=1.0, linestyle="--", alpha=0.8, label="-3 dB"
    )
    if f_cutoff is not None:
        ax.axvline(
            f_cutoff,
            color=plot_config.CONCEPT_PALETTE["accent_b"],
            linewidth=1.0,
            linestyle=":",
            alpha=0.8,
            label=f"$f_{{-3\,\mathrm{{dB}}}}$ = {f_cutoff:.3f} Hz",
        )

    ax.fill_between(freqs, H_db, -80, alpha=0.10, color=plot_config.CONCEPT_PALETTE["accent_a"])
