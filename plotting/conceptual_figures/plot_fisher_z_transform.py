...
def plot_sampling_distributions(sim_results: dict) -> plt.Figure:
    ...
    for row, rho in enumerate(RHO_SHOWCASE):
        ...
        ax_r.axvline(
            rho, color="black", linestyle="--", linewidth=1.2, label=rf"$\rho = {rho}$"
        )
...
def plot_sample_size_effect(rng) -> plt.Figure:
    ...
    for ax, n in zip(axes, SAMPLE_SIZES):
        ...
        ax.hist(
            z_s, bins=55, density=True, color=plot_config.CONCEPT_PALETTE["accent_a"], alpha=0.6, edgecolor="none"
        )
        ax.plot(
            x_theory,
            scipy_norm.pdf(x_theory, loc=z_true, scale=z_std_theory),
        ...
        ax.axvline(z_true, color=plot_config.CONCEPT_PALETTE["accent_b"], linewidth=1.0, linestyle=":")
        ax.set_title(rf"$n = {n}$")
