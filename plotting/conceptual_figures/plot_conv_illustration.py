...
def generate_conv_plot():
    ...
    ax.add_collection3d(Poly3DCollection(f_in, facecolors=plot_config.CONCEPT_PALETTE["pale"], edgecolors='k', alpha=0.3, linewidths=0.5))
    ax.add_collection3d(Poly3DCollection(f_out, facecolors=plot_config.CONCEPT_PALETTE["accent_a"], edgecolors='k', alpha=0.3, linewidths=0.5))
    ax.add_collection3d(Poly3DCollection(rf, facecolors=plot_config.CONCEPT_PALETTE["accent_b"], edgecolors='k', alpha=0.6, linewidths=1.0))
    ax.add_collection3d(Poly3DCollection(px, facecolors=plot_config.CONCEPT_PALETTE["accent_b"], edgecolors='k', alpha=0.6, linewidths=1.0))
    ...
    legend_elements = [mpatches.Patch(facecolor=plot_config.CONCEPT_PALETTE["accent_b"], edgecolor='k', alpha=0.6, label='Kernel Application')]
