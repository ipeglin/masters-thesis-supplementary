...
def plot_modes_distribution():
    ...
    if shade_start is not None:
        ...
        ax_freq.axvspan(shade_start - 1, shade_end - 1, color='#EEEEEE', alpha=0.5, zorder=-1)
        
    fig_freq.tight_layout()
    out_path_freq = out_dir / f"{get_name()}_freq.pdf"
    fig_freq.savefig(out_path_freq, bbox_inches='tight')
    plt.close(fig_freq)
    print(f"Saved {out_path_freq}")
    ...
    if shade_start is not None:
        ax_energy.axvspan(shade_start - 1, shade_end - 1, color='#EEEEEE', alpha=0.5, zorder=-1)
        
    fig_energy.tight_layout()
    out_path_energy = out_dir / f"{get_name()}_energy.pdf"
    fig_energy.savefig(out_path_energy, bbox_inches='tight')
    plt.close(fig_energy)
    print(f"Saved {out_path_energy}")
