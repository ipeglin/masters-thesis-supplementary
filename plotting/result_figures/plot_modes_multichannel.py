...
SLOW_BANDS = [
    ("Slow 5", 0.01, 0.027, "#E8F5F2"),   # pale teal (from RESULTS R1 #009E73)
    ("Slow 4", 0.027, 0.073, "#F5EFF7"),   # pale purple (from RESULTS R2 #CC79A7)
    ("Slow 3", 0.073, 0.198, "#FDFCE8"),   # pale yellow (from RESULTS R3 #F0E442)
    ("Slow 2*", 0.198, 0.200, "#F2EEF5"),  # pale wine (from RESULTS R5 #882255)
]
...
def plot_all_channels(FILE_PATH, group_name, out_filename, random_subset=False, n_channels=10, seed=42, filter_mode="all", fixed_channel_indices=None, channel_labels=None):
    ...
    fig, axes = plt.subplots(len(valid_k), C, figsize=(plot_config.FIG_WIDTH_INCHES, len(valid_k) * 0.9), sharex=True, sharey='row', squeeze=False)
    ...
    for i, k in enumerate(valid_k):
        ...
        for c in range(C):
            ...
            if c == 0:
                ax.set_ylabel(f"IMF$_{{{k}}}$\n{center_freqs[k]:.4f} Hz", rotation=0, labelpad=30, va='center')
                ax.spines['left'].set_visible(True)
                ax.tick_params(left=True, labelleft=True)
                
            if c == C - 1 and band_name:
                ax.text(1.02, 0.5, band_name, transform=ax.transAxes, va='center', ha='left',
                        # fontsize=10,
...
def plot_roi_channels(FILE_PATH, group_name, out_filename, random_subset=False, n_channels=10, seed=42, filter_mode="all"):
    ...
    for i, k in enumerate(valid_k):
        ...
        for c in range(C):
            ...
            if i == 0:
                ax.set_title(f"ROI {channel_indices[c]}\n{roi_labels[c].replace('_', ' ').replace('17networks ', '')}", 
                            #  ha='left'
                             )
