...
def load_results(directory):
    ...
    for col in ['test_acc', 'test_sens', 'test_spec', 'test_cm', 'val_acc', 'val_sens', 'val_spec', 'val_cm']:
...
def plot_confusion_matrices(
    ...
    for idx, (_, row) in enumerate(df.iterrows()):
        ax = axes[idx]
        raw_cm = row['test_cm']
        if raw_cm is None or (not isinstance(raw_cm, (list, np.ndarray))) or (isinstance(raw_cm, float) and np.isnan(raw_cm)):
